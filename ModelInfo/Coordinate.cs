...
namespace ModelInfo
{
    ...
    public class CoordinateConverter
    {
        ...
        */
        ...
        public static void LatLonToUTMXY(double lat, double lon, out double x, out double y, out int zone)
        {
   ...
   //             Console.WriteLine(x[i] + "\t" + y[i]);
        ...
        public static void LatLonOffest(double latin, double lonin, double yaw, double angleOff, double disOff, out double latout, out double lonout)
        {
            ...
            LatLonToUTMXY(latin, lonin, out x, out y, out zone);
            xd = x + disOff * Math.Cos(a);
            yd = y + disOff * Math.Sin(a);
            UTMXYToLatLon(xd, yd, out latout, out lonout, zone, latin<0);
        }
